...
const handleSubmit=()=>{
  if(SCourse!="" &&ExamName!="" &&Grade!=="" &&ExamType!="" &&StartTime.getDate()==EndTime.getDate() && StartTime.getFullYear()==EndTime.getFullYear() && StartTime.getMonth()==EndTime.getMonth())  
  {
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    sessionStorage.setItem("CID", SCourse)
...
return(
    ...
    <Select style={{width:"100%"}} value={SCourse} onChange={(event)=>setSCourse(event.target.value)}>
    {Courses.map((f)=><MenuItem value={f.ID} key={f.id} f={f}>{f.ID} </MenuItem>)}
    </Select>
    </FormControl>
    </p>
        <p>
            <Button style={{wGradeth:"100%"}} variant="contained"  onClick={()=>{handleSubmit()}}>Next Step</Button>
        </p>
          </form>
        </div>
        </div>
